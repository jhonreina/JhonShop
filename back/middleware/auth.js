...
//verificar si estamos autenticados,(existencia y verecidad del token)
...
exports.authorizeRoles=(...roles) => {
    return (req, res, next) => {
        if (!roles.includes(req.user.role)) {
            return next(new ErrorHandler(`Role(${req.user.role}) No esta autorizado para entrar a esta area`),403)
        }
        next()
    }
}